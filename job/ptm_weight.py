...
Sau khi hoàn tất, gửi notify đến CHAT_INFO.
DRY_RUN: chạy full pipeline nhưng không ghi file, không gửi.
"""
...
from config.settings import N_DAY_RUNNING, PTM_DEVICES, IS_DRY_RUN
...
def run() -> dict:
    ...
    # ── Step 4–7: Clean → Herd → Merge → Classify ────────────────────────────
    vprint("\n── Step 4–7: Clean / Herd / Merge / Classify ────────────────────")
    ...
    if not IS_DRY_RUN and tg.BOT_TOKEN and tg.CHAT_INFO:
        tg.send_telegram_message(
            ...
            f"• Dòng mới: {len(df_final):,}\n"
            f"• Master: {len(df_master):,}\n"
            f"• Herd source: {herd_source}\n"
            f"• Thời gian: {dur}s",
        )

    print(f"\n✅ PTM Weight done | {len(df_final):,} dòng mới | master: {len(df_master):,} | {dur}s")
    return {"status": "completed", "rows_new": len(df_final),
            "rows_master": len(df_master), "herd_source": herd_source}
